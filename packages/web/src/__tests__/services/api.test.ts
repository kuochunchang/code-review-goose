  ...
  describe('reviewApi', () => {
    describe('createReview', () => {
      it('should create review successfully', async () => {
        const reviewData = {
          filePath: 'test.ts',
          analysis: { issues: [], summary: 'OK' },
          notes: 'Test notes',
        };

        const mockReview = { id: '123', ...reviewData, createdAt: '2025-11-08' };

        mockApi.post.mockResolvedValue({
          data: { success: true, data: mockReview },
        });

        const result = await reviewApi.createReview(reviewData as any);

        expect(result).toEqual(mockReview);
        expect(mockApi.post).toHaveBeenCalledWith('/reviews', reviewData);
      });
    });

    describe('listReviews', () => {
      it('should list reviews without query', async () => {
        ...
        expect(mockApi.get).toHaveBeenCalledWith('/reviews', { params: {} });
      });
      ...
      it('should list reviews with filters', async () => {
        ...
        mockApi.get.mockResolvedValue({
          data: { success: true, data: { reviews: [], total: 0 } },
        });

        await reviewApi.listReviews(query as any);
    ...
    describe('getReview', () => {
      it('should get review by ID', async () => {
        const mockReview = { id: '123', filePath: 'test.ts' };

        mockApi.get.mockResolvedValue({
          data: { success: true, data: mockReview },
        });

        const result = await reviewApi.getReview('123');

        expect(result).toEqual(mockReview);
        expect(mockApi.get).toHaveBeenCalledWith('/reviews/123');
      });
    });

    describe('updateReview', () => {
      it('should update review', async () => {
        const updates = { notes: 'Updated notes', bookmarked: true };
        const mockReview = { id: '123', ...updates };

        mockApi.patch.mockResolvedValue({
          data: { success: true, data: mockReview },
        });

        const result = await reviewApi.updateReview('123', updates);

        expect(result).toEqual(mockReview);
        expect(mockApi.patch).toHaveBeenCalledWith('/reviews/123', updates);
      });
    });
    ...
    describe('deleteReview', () => {
      it('should delete review', async () => {
        ...
        await expect(reviewApi.deleteReview('123')).resolves.not.toThrow();
        expect(mockApi.delete).toHaveBeenCalledWith('/reviews/123');
      });
    });

    describe('getStats', () => {
      it('should get review stats', async () => {
        const mockStats = { total: 10, byCategory: {} };
    ...
    describe('exportReviews', () => {
      it('should export reviews in specified format', async () => {
        ...
        const result = await reviewApi.exportReviews('markdown');

        expect(result).toEqual(mockBlob);
        expect(mockApi.get).toHaveBeenCalledWith('/reviews/export', {
          params: { format: 'markdown' },
          responseType: 'blob',
        });
      });

      it('should export reviews with options', async () => {
        ...
        await reviewApi.exportReviews('html', options);
