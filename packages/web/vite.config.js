...
var backendPort = '3456';
// Try to read from test config file first (for E2E tests)
var testConfigPath = path.join(__dirname, 'e2e/.test-config.json');
if (existsSync(testConfigPath)) {
    try {
        var testConfig = JSON.parse(readFileSync(testConfigPath, 'utf-8'));
        if (testConfig.backendPort) {
            backendPort = String(testConfig.backendPort);
            console.log("[Vite] Using backend port from test config: ".concat(backendPort));
        }
    }
    catch (error) {
