...
export class OpenAIProvider implements AIProvider {
    ...
    try {
      ...
      const requestParams: any = {
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional code reviewer. Analyze code and provide detailed feedback in JSON format.',
          },
          {
            role: 'user',
            content: prompt,
  ...
  private supportsJsonMode(): boolean {
    ...
    return jsonModeModels.some(
      (supportedModel) =>
        this.model === supportedModel || this.model.startsWith(supportedModel)
    );
  }
  ...
  private supportsCustomTemperature(): boolean {
    ...
    const isRestricted = noCustomTempModels.some(
      (restrictedModel) =>
        this.model === restrictedModel || this.model.startsWith(restrictedModel)
    );
...
Provide specific, actionable feedback. Focus on the most important issues.`;
  ...
  private normalizeResult(result: any): AnalysisResult {
    return {
      issues: (result.issues || []).map((issue: any): Issue => ({
    ...
    try {
      ...
      const requestParams: any = {
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert in software architecture and UML diagrams. Generate valid Mermaid diagram syntax based on code analysis.',
          },
          {
            role: 'user',
            content: prompt,
